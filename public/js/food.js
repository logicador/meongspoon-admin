...
function getFoodCategory1List(callback) {
    fetch('/api/food/category1/get/all')
    .then((data) => { return data.json(); })
    .then((response) => {
        if (response.status != 'OK') {
            alert('에러가 발생했습니다.');
...
function removeFoodCategory1(fc1Id) {
    ...
    fetch('/api/food/category1/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function removeFoodCategory2(fc2Id) {
    ...
    fetch('/api/food/category2/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
